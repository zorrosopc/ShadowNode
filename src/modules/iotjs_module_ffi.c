...
JS_FUNCTION(FFIPrepCif)
{
  ...
  ffi_type **arg_types = sdffi_jarr_to_ffi_type_arr_ptr(jval_arg_types, (uint32_t)num_args);
  ...
  status = ffi_prep_cif(cif_ptr, abi, (unsigned int)num_args, rtype, arg_types);
  if (status == FFI_OK)
  {
...
jerry_value_t InitFFI()
{
  jerry_value_t ffi = jerry_create_object();
  
  iotjs_jval_set_method(ffi, "ffi_prep_cif", FFIPrepCif);
  iotjs_jval_set_method(ffi, "ffi_call", FFICall);
  iotjs_jval_set_method(ffi, "ffi_call_async", FFICallAsync);

#define WRAP(name)                                             \
  do                                                           \
  {                                                            \
    jerry_value_t wrap_##name = wrap_ptr(name);                \
    iotjs_jval_set_property_jval(ffi, #name, wrap_##name); \
...
#define WRAP_PREPROCESSOR_VAL(flag)                    \
  ...
  {                                                    \
    ...
    iotjs_jval_set_property_jval(ffi, #flag, val); \
